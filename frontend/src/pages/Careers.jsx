...
import { Briefcase, MapPin, Clock, ArrowRight, Loader2, Search, CheckCircle2 } from 'lucide-react';
...
const Careers = () => {
  ...
  // For a logged-in applicant: { jobId: applicationId } so we can badge roles
  // they've already applied to and link straight to that application.
  const [appliedByJob, setAppliedByJob] = useState({});
  ...
  useEffect(() => {
    if (!applicant) { setAppliedByJob({}); return; }
    portalApi.get('/applications')
      .then((res) => {
        if (res.data.success) {
          const map = {};
          res.data.data.forEach((a) => { if (a.job?._id) map[a.job._id] = a._id; });
          setAppliedByJob(map);
        }
      })
      .catch(() => { });
  ...
  // The jobs list is shared between the public page and the in-portal view.
  const jobsList = (
    loading ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filtered.map((j) => {
          const appId = appliedByJob[j._id];
          const applied = Boolean(appId);
          return (
            <Link
              key={j._id}
              to={applied ? `/portal/applications/${appId}` : `/careers/${j._id}`}
              className={`group p-6 bg-white/80 dark:bg-[#151210]/60 border rounded-none hover:shadow-[0_10px_30px_rgba(197,168,128,0.06)] transition-all duration-300 flex flex-col justify-between ${applied ? 'border-emerald-500/30' : 'luxury-border-thin hover:border-[#c5a880] dark:hover:border-[#c5a880]'}`}
            >
              <div>
                ...
                <div className="flex items-center gap-2 mt-3 flex-wrap">
                  <h3 className="text-sm font-semibold uppercase tracking-[0.12em] text-[#1c1c1c] dark:text-[#f5efe9] group-hover:text-[#c5a880] transition-colors duration-200">
                    {j.title}
                  </h3>
                  {applied && (
                    <span className="inline-flex items-center gap-1 text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">
              ...
              <div className="mt-6 pt-4 border-t luxury-border-thin flex items-center justify-between text-[9px] font-medium tracking-[0.15em] uppercase">
                ...
                <span className="inline-flex items-center gap-1 text-[#c5a880] font-semibold group-hover:translate-x-1 transition-transform duration-200">
                  {applied ? 'View Application' : 'View & Apply'} <ArrowRight size={11} />
                </span>
              </div>
            </Link>
          );
  ...
  // Logged-in applicants stay INSIDE the portal chrome (header with dashboard /
  // sign-out) so browsing jobs never feels like being logged out.
  if (applicant) {
    return (
      <PortalShell>
        <div className="mb-8">
          <span className="text-[9px] tracking-[0.3em] text-[#c5a880] uppercase font-bold block mb-2">Careers</span>
          <h1 className="text-2xl font-light uppercase tracking-[0.2em]">Open Positions</h1>
          ...
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full h-11 pl-10 pr-4 text-[10px] tracking-widest uppercase bg-white/60 dark:bg-black/10 border luxury-border-thin focus:outline-none focus:border-[#c5a880] text-[#1c1c1c] dark:text-[#f5efe9] placeholder-slate-400"
            />
          </div>
        </div>
        {jobsList}
  ...
  return (
    <div className="min-h-screen bg-luxury-gradient text-[#1c1c1c] dark:text-[#f5efe9] font-luxury flex flex-col justify-between">
      <div>
        ...
        {/* Hero Section */}
        <div className="relative overflow-hidden bg-[#1c1c1c] text-white border-b-2 luxury-border-gold py-12 sm:py-14">
